...
const FileShare = () => {
  ...
  const generateQRCode = () => {
    ...
    const link = `${baseUrl}?join=${currentRoom.code}${currentRoom.password ? `&p=${encodeURIComponent(currentRoom.password)}` : ''}`;
    return `https://api.qrserver.com/v1/create-qr-code/?size=200x200&format=png&bgcolor=ffffff&color=000000&data=${encodeURIComponent(link)}`;
  };
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    ...
                    <div className="relative group">
                      ...
                      <div className="absolute top-full mt-2 right-0 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-20">
                        <div className={`p-4 rounded-lg shadow-2xl border ${darkMode ? 'bg-gray-800 border-gray-600' : 'bg-white border-gray-200'}`}>
                          <img 
                            src={generateQRCode()} 
                            alt="Room QR Code" 
                            className="w-40 h-40 block rounded-md border border-gray-200" 
                            onError={(e) => {
                              e.target.style.display = 'none';
                              e.target.nextSibling.style.display = 'block';
                            }}
                          />
                          <div className="hidden w-40 h-40 flex items-center justify-center bg-gray-100 rounded-md text-gray-500 text-sm text-center">
